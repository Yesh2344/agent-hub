...
#   openclaw     -- Copy workspaces to ~/.openclaw/agent-hub/
...
tool_label() {
  case "$1" in
    ...
    openclaw)    printf "%-14s  %s" "OpenClaw"     "(~/.openclaw/agent-hub)" ;;
...
install_openclaw() {
  ...
  local dest; dest="$(resolve_dest openclaw "${HOME}/.openclaw/agent-hub")"
  local count=0
...
ensure_hermes_plugin_enabled() {
  ...
  plugin="agent-hub-router"
  mkdir -p "$hermes_home"
  backup="${config}.bak.agent-hub-plugin.$$"
...
install_hermes() {
  local src="$INTEGRATIONS/hermes/agent-hub-router"
  local hermes_home; hermes_home="$(hermes_home_dir)"
  local dest; dest="$(resolve_dest hermes "${hermes_home}/plugins/agent-hub-router")"
  ...
  # the agent-hub-router subdir so we never rm -rf a shared plugins dir that
  # holds other plugins.
  if [[ "$(basename "$dest")" != "agent-hub-router" ]]; then
    dest="${dest%/}/agent-hub-router"
  fi
  [[ -f "$src/plugin.yaml" && -f "$src/__init__.py" && -f "$src/data/agents.json" ]] || {
    err "integrations/hermes/agent-hub-router missing. Run ./scripts/convert.sh --tool hermes first."
    return 1
  }
  mkdir -p "$(dirname "$dest")"
  # Safety net: only ever remove our own plugin directory, never a parent.
  if [[ "$(basename "$dest")" != "agent-hub-router" ]]; then
    err "Hermes: refusing to remove '$dest' — expected an agent-hub-router directory."
    return 1
  fi
  rm -rf "$dest"
  if $USE_LINK; then
